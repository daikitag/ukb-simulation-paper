...
def main():
    ...
    N = 1000
    threshold = 30
    xaxis_single = np.linspace(0, 3, num=50)

    simulation_result_n_1 = np.zeros(50)
    # Simulation result
    for _ in range(100):
        strong_selection_df = single_ts_simulate_v(ts, w2, N, threshold, seed)
        simulation_result_n_1 += compute_proportion(
            strong_selection_df["scaled_v"], xaxis_single
        )
        seed += 1

    simulation_result_n_1 /= 100

    result_df = pd.DataFrame({"xaxis": xaxis_single, "data": simulation_result_n_1})

    result_df.to_csv(
        f"var_propseed_{seed}_n_1_underdominance_popsize_1000_U_1_S_10_seed_25.csv",
        index=False,
    )
